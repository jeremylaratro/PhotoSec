...
def start():
    ...
    //         .---.            //                                                                                                                          //
   //          |[X]|           // `_______```__````````````````````__`````````````````______```````````````````````__````__````__``````__``__``````````````// 
   //   _.==._.""""".___~__    // /```````\`/``|``````````````````/``|```````````````/``````\`````````````````````/``|``/``|``/``|````/``|/``|````````````//
   //  d __ ___.-''-. _____b   //  $$$$$$$``|$$`|____````______```_$$`|_`````______``/$$$$$$``|``______````_______`$$`|``$$`|`_$$`|_```$$/`$$`|``_______` //
   //  |[__]  /."__".\ _   |   //  $$`|__$$`|$$``````\``/``````\`/`$$```|```/``````\`$$`\__$$/``/``````\``/```````|$$`|``$$`|/`$$```|``/``|$$`|`/```````| //
   //  |     /  /""\  \ (_)|   //  $$````$$/`$$$$$$$``|/$$$$$$``|$$$$$$/```/$$$$$$``|$$``````\`/$$$$$$``|/$$$$$$$/`$$`|``$$`|$$$$$$/```$$`|$$`|/$$$$$$$/` //
   //  |     \  \__/  /    |   //  $$$$$$$/``$$`|``$$`|$$`|``$$`|``$$`|`__`$$`|``$$`|`$$$$$$``|$$````$$`|$$`|``````$$`|``$$`|``$$`|`__`$$`|$$`|$$``````\` //
  ...
  //    `-----------------'   //  $$/```````$$/```$$/``$$$$$$/`````$$$$/```$$$$$$/```$$$$$$/```$$$$$$$/``$$$$$$$/``$$$$$$/`````$$$$/``$$/`$$/`$$$$$$$/`` //
   ...
   ---------------------------------------------------------------------------------------------------------------------------------- V0.2.0 -------------
    ...
    [+] This script contains functions to bulk clear EXIF data, check for GPS data, rename files and perform image analysis with popular tools into a single output. 
    [+] Please note that this program will overwrite the original files. Make sure you have a backup of the files before running this program
